...
const typeNum = (arr) => {
  // Solution code here...
  let onlyNums = arr.filter(element => typeof element === 'number');
...
const containsAnd = (arr) => {
  ...
  let newString = arr.filter((arr => arr.includes('and')));
...
const oddValues = (arr) => {
  // Solution code here...
  let odds = arr.filter((arr) => arr % 2 === 1);
...
const notInFirstArray = (forbiddenValues, arr) => {
  // Solution code here...
  let newArray = arr.filter(values => !forbiddenValues.includes(values));
