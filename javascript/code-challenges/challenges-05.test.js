...
const addValues = (arr) => {
  ...
  const values = arr.reduce(
    (accumulator, curValue) => accumulator + curValue, index);
...
const addPurchases = (arr) => {
  // Solution code here...
  let newPrice = arr.reduce((acc, curVal) => {
...
const countNumberOfElements = (arr) => {
  // Solution code here...
  let elementNum = arr.reduce((acc, curVal) => {
    acc = curVal;
    return acc;
  }, []);
  return elementNum;
...
const reversedString = (str) => {
  ...
  let reversedString = newArr.reduce((accumulator, curVal) => {
